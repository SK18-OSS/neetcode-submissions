class Hero:
    def __init__(self, name: str, base_power: float, attributes: dict):
        self.name = name
    ...
    def calculate_effective_power(base_power, attributes):
        attribuite_bonus = sum(attributes.values())/len(attributes)
        effective_power = round(base_power * (1 + attribuite_bonus), 1)
